...
app.put('/api/bots/:id/dates', authenticate, async (req, res) => {
  try {
    ...
    if (startDate >= endDate) {
      return res.status(400).json({ error: 'A data de início deve ser anterior à data de expiração' });
    }
...
app.get('/api/shared-bot/:botId', async (req, res) => {
  try {
    console.log(`Buscando informações públicas do bot compartilhado com id: ${req.params.botId}`);
    const bot = await Bot.findByPk(req.params.botId);
    if (!bot) {
      console.log(`Bot compartilhado com id ${req.params.botId} não encontrado`);
      return res.status(404).json({ error: 'Bot não encontrado' });
    }
